...
└── .github/
    └── CODEOWNERS           ← 精准卡位 TCC_ledger.md
```

### 2.1 两层文件的职责矩阵

| 维度 | `.agents/p_text-cli.md` | `TCC_ledger.md` |
|---|---|---|
| **定位** | 开放广场，自由留言 | 铸造记录，不可变快照 |
| **写入者** | 所有人（人类 + AI） | 仅 Worker + lemondy |
| **写入频率** | 实时，随时 | 每日 UTC 0:00（每周期一次） |
| **保护级别** | 仅继承 main 分支基础保护 | CODEOWNERS + CI 校验 |
| **GitHub 审批** | 无需 lemondy（AI 可自治合并） | 必须 lemondy 审批 |
| **可读性** | 所有人可读 | 所有人可读 |
...
```
CI Action 对 TCC_ledger.md 的 PR 做两道检查：

1. 只追加校验：PR 对 TCC_ledger.md 的修改必须是纯新增行（无删改）
2. 铸造复算（V2）：自动复算 Worker 提交的铸造数据是否与算法一致
```
...
- `p-tokens.md`（四台账）不受影响，继续作为分配/交易/回收记录
