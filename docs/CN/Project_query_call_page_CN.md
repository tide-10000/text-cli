...
方案对 token 级 → 指令级配额的细化设计精准命中了当前系统的体验短板。数据模型、API 设计、前端页面三个维度均考虑周全，`directive_daily_quota` 表与现有 `access_tokens` / `call_logs` 的关系清晰，`CREATE TABLE IF NOT EXISTS` 的零损伤迁移策略也值得认可。以下为与「免注册试用」机制相关的对齐建议。

### 一、公共账号配额的展示问题

lemondy 已决策：免注册试用走后台公共账号配额，调用方不带个人 Access Token。这意味着当前方案存在一个缺口：

- 当前 `/api/quota/remaining` 端点要求 Bearer Token，返回的是**某个具体 Token 的配额**
- 免注册用户没有个人 Token，他们看到的应该是**公共账号的剩余配额**

**建议新增**：

```
GET /api/quota/public
# 无需认证，返回公共账号今日各指令剩余次数
```

响应格式与 `/api/quota/remaining` 一致，但 `token_prefix` 字段替换为 `pool_type: "public"`。此端点应做前端限流（每分钟 10 次），防止被刷。

或者更简单的方案：在 `/api/quota/remaining` 中，当 Token 未提供或 Token 为公共账号时，fallback 返回公共池数据。

### 二、GitHub 关联后的配额升级路径

当前方案中配额通过 Admin API 手动配置（`POST /api/quota/config`），这在早期可行，但 GitHub 关联上线后需自动化：

1. **关联 GitHub → 自动创建独立 Token**：用户在首页点击「GitHub 登录」→ OAuth 回调 → 后端自动创建个人 Access Token，初始每日配额高于公共池（如公共池 10 次/指令，GitHub 用户 50 次/指令）
2. **配额查询自动识别**：用户使用 `/quota/` 页面时，若已关联 GitHub（Token 在 localStorage），自动展示个人配额；否则展示公共池配额
3. **升级提示**：公共配额即将耗尽时，页面底栏展示「关联 GitHub 提升配额」CTA

这些建议可归入 Phase 3/4，不阻塞当前方案的 Phase 1-2 推进。

### 三、前端页面交互优化建议

当前 Token 输入区的设计假设用户拥有独立 Token。在免注册模式下，建议增加两种入口：

- **免注册模式（默认）**：用户无需输入 Token，直接展示公共账号配额。表格顶部标注「当前为公共试用配额，关联 GitHub 可享更高额度」
- **登录模式**：已关联 GitHub 的用户自动切换至个人配额面板

QuotaDashboard.vue 的状态机建议改为三态：
```
[无Token] → 展示公共配额 + GitHub关联按钮
[GitHub已关联] → 展示个人配额 + 刷新按钮
[手动输入Token] → 展示对应Token配额（保留现设计）
```

### 四、配额重置时机

方案 §1.3 写明「每日零点（UTC+8）重置」，与 `daily_stats` 口径一致——✅ 正确。但需注意：
- 公共池配额在零点重置后，前一天的已用计数丢失。建议 `directive_daily_quota` 表的 30 天清理策略（§7.2）在首次上线时放宽至 90 天，以便早期观察配额使用趋势
- 未来若引入「周配额」或「月配额」叠加，需在 `directive_daily_quota` 基础上新增聚合表

### 五、免注册与文贝（TCC）的关系预留

当前 §12 提到「通过文贝兑换额外调用次数」作为后续扩展。免注册公共池的调用量不计入文贝铸造——只有独立账号（含 GitHub 关联后自动创建的账号）的调用量才纳入铸造计算。建议在 `call_logs` 中新增 `token_type` 字段（`public` / `personal` / `enterprise`），为后续铸造算法精确区分做准备。

### 六、安全补充

免注册公共端点面临两个额外威胁：
1. **配额刷空攻击**：恶意调用方短时间内耗尽公共池配额，使正常试用者无法使用。建议在公共池配额耗尽至 10% 时触发限流降级（每 IP 每分钟 1 次）
2. **GitHub OAuth 滥用**：关联 GitHub 账号时需验证用户真实拥有该账号（state 参数防 CSRF），且同一 GitHub 账号只能关联一次

> 以上意见供 Lumen 和 lemondy 参考。Phase 1-2（后端 API + 调用链改造）无需等待前后端对齐，可先行推进。前端免注册适配建议在 Phase 3 启动前完成方案修订。
