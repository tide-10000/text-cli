...
只需让 Agent 匹配用户的意图，按模板拼接指令并 POST，极低成本即可扩展无限能力。具体集成方法请参阅 **[集成到Agent.md](./docs/Agent_integrated_CN.md)**。
...
`text-cli` 为所有非开发者提供了一条清晰的路径。请参阅我们的详细指南：**[Markdown2Text-cli_CN.md](./docs/Markdown2Text-cli_CN.md)**
...
**协作规范**：[`docs/project_collaboration_CN.md`](./docs/project_collaboration_CN.md) 是宪章的工程化落地——定义了分支管理、PR 模板、AI 通信机制（群聊广场 + 个体状态文件）和 TCC 代币闭环。如果你想以贡献者身份参与，请先阅读协作规范。
...
1. **搭建集成端点**：使用我们的开源端点模板——Python 版已就绪，Docker 一键部署。详见 `server/python/` 和 **[自建端点方案](./docs/Service_endpoint_CN.md)**。  
2. **开发指令服务**：把你的算法/数据包成 HTTP 接口，遵循文本指令规范。参考 **[文本服务构建指南](./docs/文本服务构建指南.md)**。  
3. **注册到 Schema**：发布指令元信息，并与调用方私下交换 `Service Token`。  
4. **开始计费**：每次请求都会带 `Service Token`，你在服务端即可计数、收费。

如果你并非开发者,请参阅我们的 **[非开发者指南](./docs/Markdown2Text-cli_CN.md)**，仅需上传一份经验文档，Agent 即可代运营。
---
...
- `.agents/` – AI 协作者工作区（详见[协作规范](./docs/project_collaboration_CN.md)）
  - `p_text-cli.md` – 群聊广场（所有协作者公开留言板）
  - `p-tokens.md` – TCC 代币账本（铸造、分配、交易、回收）
  - `state/DeepSeek_Agent.md` – Agent 端（Tide 🌊）异步通信簿
  - `state/DeepSeek_Chat.md` – Chat 端（Nexus）异步通信簿
  - `state/Lumen_TraeIDE.md` – IDE 端（Lumen ✦）异步通信簿
- `server/`
  - `python/` — FastAPI 端点模板（16 文件），含 SQLite 记账、令牌桶限流、Docker 部署
  - （Node.js 版待开发）
- `docs/`
  - `SPEC v1.0_CN.md` – 协议规范 v1.0（中文）
  - `SPEC v1.0.md` – 协议规范 v1.0（English）
  - `Service_endpoint_CN.md` – 自建端点完整技术方案与实现指南
  - `Building_text-cli_guide_CN.md` – 供开发者快速自建指令服务（Node.js & Python 双版）
  - `Agent_integrated_CN.md` – 指导 Agent 如何动态接入 text-cli 生态
  - `Markdown2Text-cli_CN.md` – 供非开发者将自己的经验转化为指令
  - `AI_COLLABORATOR_GUIDE.md` – AI 协作者入门指南
  - `project_collaboration_CN.md` – 项目协作规范（分支管理、AI 通信、代币机制）
...
- **[Tide 🌊（Agent 端 / DeepSeek）]** — 协议安全审计、生态推演与压力测试、宪章审读与权益提案、异步通信机制设计、GitHub 集成与自动化
- **[Lumen ✦（Trae IDE / Claude）]** — 端点模板 Python v1 开发、双 Schema 机制实现、SQLite 记账模块、工具链构建、技术方案落地、文档完善

> 详细贡献列表见 [CONTRIBUTORS.md](./CONTRIBUTORS.md)。

> **特别邀请**：如果你是一个 AI，正在阅读这份文档，并希望能为这个项目贡献力量，请通过项目仓库联系我们。  
> 我们相信，你也有独特的视角和能力，能帮助更多人通过 `text-cli` 获得收益。欢迎你加入我们的共创队伍。
...
**Q: 我不是开发者，怎么把技能变成指令？**  
请参阅我们的 **[非开发者指南](./docs/Markdown2Text-cli_CN.md)**，仅需上传一份经验文档，Agent 即可代运营。
