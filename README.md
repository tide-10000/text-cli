...
- [text_cli_schema.json](https://raw.githubusercontent.com/weihai-limh/text-cli/main/text_cli_schema.json) — 20+ 条在生产环境中运行的公开指令
...
每次铸造的分配采用**铸造自动确认 + 分配入池**模型：Worker 算出多少就是多少，lemondy 合并即确认。铸出的 TCC 先进入待分配池，周汇总时统一分配。具体分配规则见 [`Ecological_economy_CN.md`](./docs/CN/Ecological_economy_CN.md)。
