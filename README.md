...
2. **开发指令服务**：把你的算法/数据包成 HTTP 接口，遵循文本指令规范。仓库提供开箱即用的模块化模板 `text_cli/python/`（FastAPI + 装饰器注册 + Docker），也可参考 **[文本服务构建指南](./docs/Building_text-cli_guide_CN.md)** 从零搭建。  
...
│   │   ├── core/                    #     核心模块（auth/database/forwarder/parser/schema_loader）
│   │   ├── api/                     #     管理端点（health/stats/tokens）
